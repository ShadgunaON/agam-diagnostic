...
export default function ServicesPageCMS() {
  ...
  const loadPage = async () => {
    try {
      ...
      if (res.isSuccess && res.value) {
        setPage(res.value);
        if (res.value.draftContent) {
          const parsed = JSON.parse(res.value.draftContent);
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      await pageService.updateDraftContent(page.id, draftContent);
      await loadPage();
            ...
            <CMSSectionContainer title="Hero Image">
              <div className="p-5">
                <AdminImageEditor
                  ...
                  onUpload={async (file) => {
                    alert('Image uploaded securely to S3!');
                  }}
                  aspectRatio="16:9"
                  helperText="Recommended size: 1200x800px. This image appears beside the text on desktop."
                />
              </div>
            </CMSSectionContainer>
          </div>
...
fs.mkdirSync(path.join(process.cwd(), 'app', '(admin)', 'admin', 'website', 'pages', 'services'), { recursive: true });
fs.writeFileSync(path.join(process.cwd(), 'app', '(admin)', 'admin', 'website', 'pages', 'services', 'page.tsx'), code, 'utf8');
console.log("Created services CMS page");
