...
export default function MenHealthCMSPage() {
  ...
  const loadPage = async () => {
    try {
      setLoading(true);
      const res = await pageService.getPageById('men-health');
      if (res) {
        setPage(res);
        if (res.draftContent) {
      ...
      console.error('Failed to load men health page:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveDraft = async () => {
    setSaving(true);
    try {
      await pageService.updatePage('men-health', JSON.stringify(content));
      await loadPage();
    } catch (err) {
  ...
  const handlePublish = async () => {
    ...
    setSaving(true);
    try {
      await pageService.publishPage('men-health', JSON.stringify(content));
      await loadPage();
    } catch (err) {
  ...
  const renderActiveSection = () => {
    switch (activeSection) {
      case 'overview':
        return (
          <div className="space-y-6">
            <SectionHeader title="Men's Health Page Overview" description="Current status and content summary" />
      ...
      case 'hero':
        return (
          <div className="space-y-6">
            <SectionHeader title="Hero Section" description="The main banner at the top of the Men's Health page." />
      ...
      <div className="flex items-center justify-between px-6 py-4 bg-white border-b shrink-0 z-10 sticky top-0">
        <div className="flex items-center gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Manage public-facing content for the Men's Health page</p>
          </div>
        </div>
